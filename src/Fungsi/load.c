...
int LoadConfigFile(LoadConfig *config, WebDatabase *db, char *folder)
{
    FILE *file;
    char path[MAX_PATH_LENGTH];
    ...
    // BARIS 1: Konstanta Konfigurasi Global
    if (fscanf(file, "%d %d %d %d",
               &config->cache_max_amount,
               &config->tabs_max_amount,
               &config->download_max_amount,
               &config->max_web_pages) != 4)
    {
        fclose(file);
        return 0;
    }

    // BARIS 2: Status Tab Global
    if (fscanf(file, "%d %d", &db->Tab.tab_count, &db->Tab.current_tab) != 2)
    {
        fclose(file);
        return 0;
    }
    fgetc(file); // Membersihkan sisa karakter newline setelah fscanf

    // BARIS 3 & 4: Metadata TAB dan Data Riwayat URL (Looping sesuai jumlah tab)
    int i, j;
    for (i = 0; i < db->Tab.tab_count; i++)
    {
        ...
        // BARIS 3: Nama TAB, Total Riwayat, dan Pointer Posisi Riwayat
        if (fscanf(file, "%s %d %d", tab->nama_tab, &tab->web_count, &posisi_aktif) != 3)
        {
            fclose(file);
            return 0;
        }
        tab->current_web_idx = posisi_aktif - 1; // Kembalikan ke 0-indexed berbasis array C
        fgetc(file); // Membersihkan newline

        // BARIS 4: Data Riwayat URL dipisahkan spasi
        for (j = 0; j < tab->web_count; j++)
        {
            if (j == tab->web_count - 1)
            {
                // URL terakhir dibaca sampai bertemu newline
                fscanf(file, "%s", tab->daftar_web[j].web_url);
            }
            else
            {
                // URL selain terakhir dibaca dipisahkan spasi
                fscanf(file, "%s ", tab->daftar_web[j].web_url);
            }
        }
    }
...
int LoadData(WebDatabase *db, LoadConfig *config, char *folder)
{
    ...
    LoadConfigFile(&temp_config, db, folder);
